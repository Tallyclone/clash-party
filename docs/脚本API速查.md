...
**耗时是跳变的**，等于 `ceil(未命中数 / 200) × 6000ms`：1 波 6.3s / 2 波 12.6s / 3 波 17.7s。**HTTP 客户端超时给到 20 秒以上**，并靠提高 `maxAge` 提升缓存命中率。
...
程序自己每 3 小时跑一轮，另外在启动 / 订阅更新 / 切配置时触发。冷启动后表是空的，第一轮跑完（启动约 20 秒后开始）名单才有内容。
