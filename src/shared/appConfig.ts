 ...
 *
 * 这个信号量同时是「工位会被多脚本打爆、而内核测速不会」的根本原因：
 * 它让内核测速成为**弹性**资源 —— 争用的结果是排队变慢，而不是拿到错误答案。
 * 所以 mode=delay 一律走这里，不碰工位。
 *
 * ⚠ 名单探测**不走**探测模块的单飞锁，所以它可以和一轮全量基线（并发 200）同时跑，
 * 瞬时叠加最坏是 400。实测并发临界点在 200~300 之间，超过就会把健康节点打成
 * delay 0（见 DELAY_PROBE_FULL_CONCURRENCY 的对照数据）。基线是 3 小时一轮的
 * 低频事件，撞上的概率很低；真观察到「测完一轮之后可用节点数突然掉一截」，
 * 把这个数降下来即可。
 */
 ...
 * 全量基线探测并发。
 *
 * 取 200 不是拍脑袋：实测耗时严格等于「波数 × timeout」，只有离散档位，
 * 而并发开太大会把健康节点打成 delay 0。ABABA 交替对照（全量 642 @timeout=6000，
 * 交错模式已排除时间趋势）：
 ...
 * 临界点落在 200 与 300 之间。想要更快只能减少要测的节点数，不能加并发。
 */
export const DELAY_PROBE_FULL_CONCURRENCY = 200
